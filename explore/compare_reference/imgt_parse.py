...
if __name__ == "__main__":
    ...
    with FastaIterator(
        "raw_data/IMGTGENEDB-ReferenceSequences.fasta-AA-WithoutGaps-F+ORF+inframeP", full_header=True
    ) as fai:
        for record in fai:
            ...
            if not (gene_name.startswith("TRA") or gene_name.startswith("TRB")):
                continue
            seqname = f"IMGT-{split_header[1]}-{split_header[0]}"
            gene_seqs_aa[gene_name][seqname] += record.sequence
    with FastaIterator(
        "raw_data/IMGTGENEDB-ReferenceSequences.fasta-nt-WithoutGaps-F+ORF+inframeP", full_header=True
    ) as fai:
        for record in fai:
            split_header = record.seq_id.split("|")
            if not split_header[2].startswith("Homo sapiens"):
    ...
    with FastaIterator("raw_data/Homo_sapiens.GRCh38.cdna.all.fa", full_header=True) as fai:
        for record in fai:
            split_header = record.seq_id.split(" ")
            accn = split_header[0]
            gene_symbol = None
            ...
            for k_v in split_header[1:]:
                ...
                if kvs[0] == "gene_symbol":
                    gene_symbol = kvs[1]
            if gene_biotype is None or not (gene_name.startswith("TRA") or gene_name.startswith("TRB")):
                continue
            if gene_symbol is None:
                continue
            seqname = f"ENSG-{accn}"
            gene_seqs_nt[gene_symbol][seqname] += record.sequence
    with FastaIterator("raw_data/Homo_sapiens.GRCh38.pep.all.fa", full_header=True) as fai:
        for record in fai:
            split_header = record.seq_id.split(" ")
            accn = split_header[0]
            gene_symbol = None
    ...
    with FastaIterator("raw_data/uniprot_sprot.fasta", full_header=True) as fai:
        for record in fai:
    ...
    with FastaIterator("raw_data/igblast.fa", full_header=False) as fai:
        for record in fai:
            split_header = record.seq_id.split("*")
    ...
    #             for seq_id, seq in seqs.items():
    #                 faw.write(FastaRecord(seq_id, seq))
    with FastaWriter(f"all_nt.fa") as faw:
        for gene_name, seqs in gene_seqs_nt.items():
            for seq_id, seq in seqs.items():
                faw.write(FastaRecord(gene_name + "-" + seq_id, seq))
    with FastaWriter(f"all_aa.fa") as faw:
        for gene_name, seqs in gene_seqs_aa.items():
            for seq_id, seq in seqs.items():
                faw.write(FastaRecord(gene_name + "-" + seq_id, seq))
